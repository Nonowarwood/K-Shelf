...
import { updateProfile } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js"; // Ajuste la version selon ton projet

// --- INITIALISATION & UI TOGGLE ---
const settingsModal = document.getElementById('settings-modal');
const closeSettingsBtn = document.getElementById('close-settings');

// Fonction pour ouvrir/fermer (Tu pourras lier l'ouverture à un bouton engrenage de ta sidebar)
function toggleSettingsModal() {
  settingsModal.classList.toggle('hidden');
}
closeSettingsBtn.addEventListener('click', toggleSettingsModal);

// --- LOGIQUE INTERFACE (Sauvegarde locale & Variables CSS) ---
...
function loadInterfaceSettings() {
  const savedBlur = localStorage.getItem('kshelf_blur') || 12;
  const savedSpeed = localStorage.getItem('kshelf_speed') || 20;
  const savedTheme = localStorage.getItem('kshelf_theme') || 'midnight';

  // Appliquer le flou
  rangeBlur.value = savedBlur;
  valBlur.textContent = `${savedBlur}px`;
  document.documentElement.style.setProperty('--glass-blur', `${savedBlur}px`);

  // Appliquer la vitesse de l'aurora
  rangeSpeed.value = savedSpeed;
  valSpeed.textContent = `${savedSpeed}s`;
  document.documentElement.style.setProperty('--aurora-speed', `${savedSpeed}s`);
  // Note: Pense à appliquer var(--aurora-speed) sur l'animation CSS de tes bulles aurora

  // Appliquer le thème
  applyThemeGradient(savedTheme);
...
rangeBlur.addEventListener('input', (e) => {
  ...
  localStorage.setItem('kshelf_blur', val);
});

rangeSpeed.addEventListener('input', (e) => {
  const val = e.target.value;
  valSpeed.textContent = `${val}s`;
  document.documentElement.style.setProperty('--aurora-speed', `${val}s`);
  localStorage.setItem('kshelf_speed', val);
});

// Événements Thèmes
const gradients = {
  midnight: "linear-gradient(135deg, #0f172a, #1e1b4b)",
  krose: "linear-gradient(135deg, #2e1022, #4c0519)",
  cyber: "linear-gradient(135deg, #022c22, #064e3b)"
};
...
themeButtons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    applyThemeGradient(theme);
    localStorage.setItem('kshelf_theme', theme);
  });
});

function applyThemeGradient(themeName) {
  document.documentElement.style.setProperty('--bg-gradient', gradients[themeName]);
}


// --- LOGIQUE PROFIL (Firebase Auth + LocalStorage pour la Bio) ---
const btnSaveProfile = document.getElementById('btn-save-profile');
const usernameInput = document.getElementById('settings-username');
const bioInput = document.getElementById('settings-bio');

// Remplir les champs quand l'utilisateur est connecté
auth.onAuthStateChanged((user) => {
  if (user) {
    usernameInput.value = user.displayName || "";
    bioInput.value = localStorage.getItem(`kshelf_bio_${user.uid}`) || "";
  }
});
...
btnSaveProfile.addEventListener('click', async () => {
  ...
  if (!user) {
    alert("Tu dois être connecté pour modifier ton profil !");
    return;
  }

  try {
    // 1. Sauvegarde du pseudo sur Firebase Auth
    await updateProfile(user, {
      displayName: usernameInput.value.trim()
    });

    // 2. Sauvegarde de la bio (en localstorage lié à l'UID pour l'instant)
    localStorage.setItem(`kshelf_bio_${user.uid}`, bioInput.value.trim());

    alert("Profil mis à jour avec succès ! ✨");
    
    // Mettre à jour l'affichage du profil sur ton app
...
document.addEventListener('DOMContentLoaded', loadInterfaceSettings);
